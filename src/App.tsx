...
// Beadle Companion theme colors
const C = {
  primary: "#1e40af",      // deep blue
  primaryLight: "#3b82f6", // lighter blue
  primaryBg: "#dbeafe",    // very light blue
  bg: "#f1f5f9",           // page background
  card: "#ffffff",         // card white
  text: "#1e293b",         // dark text
  textMuted: "#64748b",    // muted gray
  textLight: "#94a3b8",    // lighter gray
...
function StudentSearch({ onSelect }) {
  ...
  return (
    <div style={{ position: "relative" }}>
      <input autoFocus value={query} onChange={handleChange} placeholder="Type student name or ID number..."
        style={{ width: "100%", background: C.card, border: `1.5px solid ${C.border}`, borderRadius: 10, padding: "14px 18px", fontSize: 16, color: C.text, outline: "none", boxSizing: "border-box" }} />
      {(loading || results.length > 0 || (query.length >= 2 && !loading)) && (
        <div style={{ position: "absolute", top: "calc(100% + 6px)", left: 0, right: 0, background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, zIndex: 50, overflow: "hidden", boxShadow: "0 10px 30px rgba(15,23,42,0.12)" }}>
          {loading && <div style={{ padding: "14px 18px", color: C.textMuted, fontSize: 14 }}>Searching...</div>}
          {!loading && results.map(s => (
            <div key={s.student_id} onClick={() => { onSelect(s); setQuery(""); setResults([]); }}
              style={{ padding: "12px 18px", cursor: "pointer", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}
          ...
          {!loading && results.length === 0 && query.length >= 2 && (
            <div style={{ padding: "14px 18px", color: C.textMuted, fontSize: 14 }}>No students found.</div>
          )}
        </div>
      )}
    </div>
...
function SlipPreview({ slip, onDone }) {
  return (
    <div style={{ background: C.card, borderRadius: 12, padding: "32px 36px", width: "100%", maxWidth: 480, boxShadow: "0 10px 30px rgba(15,23,42,0.08)", color: C.text, border: `1px solid ${C.border}` }}>
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 700, marginBottom: 6, color: C.textMuted, letterSpacing: 0.8 }}>NATURE:</div>
      ...
      {slip.nature.includes("Late") && (
        <div style={{ marginBottom: 14 }}>
          <span style={{ fontSize: 11, fontWeight: 700, color: C.textMuted, letterSpacing: 0.8 }}>IF LATE: </span>
          <span style={{ fontSize: 13, fontWeight: 700, marginLeft: 8, color: C.text }}>{slip.meridiem}</span>
        </div>
      )}

      {slip.reason && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 11, fontWeight: 700, marginBottom: 4, color: C.textMuted, letterSpacing: 0.8 }}>REASON:</div>
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ fontSize: 11, fontWeight: 700, marginBottom: 6, color: C.textMuted, letterSpacing: 0.8 }}>STATUS:</div>
...
function Field({ label, value, span }) {
  return (
    <div style={{ gridColumn: span ? "1 / -1" : undefined }}>
      ...
      <div style={{ fontSize: 14, fontWeight: 600, borderBottom: `1px solid ${C.border}`, paddingBottom: 4, color: C.text }}>{value || "—"}</div>
    </div>
  );
}
...
export default function App() {
  ...
  const pending = log.filter(e => !e.status).length;

  // ── Styles ──────────────────────────────────────────────────────
  const styles = {
    root: { minHeight: "100vh", background: C.bg, fontFamily: "'Inter', system-ui, sans-serif", color: C.text },
    header: { background: C.card, borderBottom: `1px solid ${C.border}`, padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 },
    logo: { display: "flex", alignItems: "center", gap: 12 },
    logoCircle: { width: 38, height: 38, background: `linear-gradient(135deg, ${C.primary}, ${C.primaryLight})`, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 14 },
    logoText: { fontSize: 17, fontWeight: 700, color: C.text },
    logoSub: { fontSize: 11, color: C.textMuted, marginTop: -2 },
    navBtn: (active) => ({ background: active ? C.primary : "transparent", color: active ? "#fff" : C.textMuted, border: `1px solid ${active ? C.primary : C.border}`, borderRadius: 8, padding: "8px 18px", fontWeight: 600, fontSize: 14, cursor: "pointer", transition: "all 0.15s" }),
    main: { maxWidth: 1100, margin: "0 auto", padding: "28px 20px" },
    greeting: { fontSize: 30, fontWeight: 800, color: C.text, marginBottom: 4 },
    subGreeting: { fontSize: 14, color: C.textMuted, marginBottom: 28 },
    statsGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 14, marginBottom: 28 },
    statCard: { background: C.card, borderRadius: 10, padding: "16px 18px", border: `1px solid ${C.border}` },
    statLabel: { fontSize: 11, fontWeight: 700, color: C.textMuted, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 4 },
    statValue: (color) => ({ fontSize: 32, fontWeight: 800, color, lineHeight: 1 }),
    statSub: { fontSize: 12, color: C.textMuted, marginTop: 4 },
    panel: { background: C.card, borderRadius: 10, padding: 24, border: `1px solid ${C.border}`, marginBottom: 16 },
    panelTitle: { fontSize: 18, fontWeight: 800, color: C.text, marginBottom: 6 },
    panelSub: { fontSize: 13, color: C.textMuted, marginBottom: 20 },
    label: { fontSize: 11, fontWeight: 700, letterSpacing: 0.8, color: C.textMuted, textTransform: "uppercase", marginBottom: 8, display: "block" },
    errMsg: { fontSize: 12, color: C.danger, marginTop: 5 },
    natureBtn: (active) => ({ padding: "11px 14px", border: `2px solid ${active ? C.primary : C.border}`, background: active ? C.primaryBg : C.card, color: active ? C.primary : C.textMuted, borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: "pointer", textAlign: "center", transition: "all 0.15s" }),
    statusPill: (st) => {
    ...
    <div style={styles.root}>
      ...
      <div style={styles.header}>
        <div style={styles.logo}>
          <div style={styles.logoCircle}>POD</div>
          <div>
            <div style={styles.logoText}>Discipline Office</div>
            <div style={styles.logoSub}>Admission Slip System</div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button style={styles.navBtn(view === "checkin" || view === "slip")} onClick={() => { setView("checkin"); resetForm(); }}>New Slip</button>
          <button style={styles.navBtn(view === "log")} onClick={() => setView("log")}>Slip Log</button>
        </div>
      </div>

      <div style={styles.main}>

        {/* Greeting + Stats only on checkin search */}
        {view === "checkin" && step === "search" && (
          <>
            <div style={styles.greeting}>{getGreeting()}, POD Officer 👋</div>
            <div style={styles.subGreeting}>{getToday()} · <Clock /></div>

            <div style={styles.statsGrid}>
              <div style={styles.statCard}>
                <div style={styles.statLabel}>Today's Slips</div>
                <div style={styles.statValue(C.primary)}>{todayLogs.length}</div>
                <div style={styles.statSub}>Recorded so far</div>
              </div>
              <div style={styles.statCard}>
                <div style={styles.statLabel}>Pending Status</div>
                <div style={styles.statValue(C.warning)}>{pending}</div>
                <div style={styles.statSub}>Awaiting POD review</div>
              </div>
              <div style={styles.statCard}>
                <div style={styles.statLabel}>Total Logged</div>
                <div style={styles.statValue(C.text)}>{log.length}</div>
                <div style={styles.statSub}>All-time entries</div>
              </div>
            </div>
          </>
        )}
        ...
        {view === "checkin" && step === "search" && (
          <div style={styles.panel}>
            <div style={styles.panelTitle}>🔍 Find Student</div>
            <div style={styles.panelSub}>Search by name or Student ID number to begin a new admission slip.</div>
        ...
        {view === "checkin" && step === "form" && (
          <div style={styles.panel}>
            <div style={{ background: C.primaryBg, borderRadius: 8, padding: "12px 16px", marginBottom: 24, display: "flex", alignItems: "center", justifyContent: "space-between", border: `1px solid ${C.primary}` }}>
              <div>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={styles.label}>Nature of Violation</label>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 }}>
                {NATURE_OPTIONS.map(n => (
                  <button key={n} style={styles.natureBtn(nature.includes(n))} onClick={() => toggleNature(n)}>{n}</button>
                ))}
              </div>
              ...
              {errors.nature && <div style={styles.errMsg}>{errors.nature}</div>}
              {errors.others && <div style={styles.errMsg}>{errors.others}</div>}
            </div>

            {nature.includes("Late") && (
              <div style={{ marginBottom: 20 }}>
                <label style={styles.label}>If Late</label>
            ...
            <div style={{ marginBottom: 24 }}>
              <label style={styles.label}>Student's Reason / Explanation</label>
              ...
              {errors.reason && <div style={styles.errMsg}>{errors.reason}</div>}
            </div>
        ...
        {view === "log" && (
          <>
            <div style={styles.greeting}>Slip Log & Status</div>
            <div style={styles.subGreeting}>Review submitted slips and assign their disposition.</div>
            ...
            <div style={styles.statsGrid}>
              ...
              <div style={styles.statCard}>
                <div style={styles.statLabel}>Excused</div>
                <div style={styles.statValue(C.success)}>{log.filter(e => e.status === "Excused").length}</div>
              </div>
              <div style={styles.statCard}>
                <div style={styles.statLabel}>Unexcused</div>
                <div style={styles.statValue(C.danger)}>{log.filter(e => e.status === "Unexcused").length}</div>
              </div>
              <div style={styles.statCard}>
            ...
            <div style={styles.panel}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 12 }}>
                <div style={{ fontSize: 16, fontWeight: 700, color: C.text }}>All Admission Slips</div>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
                {["All", ...NATURE_OPTIONS].map(f => (
                  <button key={f} onClick={() => setFilterNature(f)} style={{ background: filterNature === f ? C.primary : C.bg, color: filterNature === f ? "#fff" : C.textMuted, border: `1px solid ${filterNature === f ? C.primary : C.border}`, borderRadius: 6, padding: "5px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>{f}</button>
                ))}
              </div>
                    ...
                    <tbody>
                      {log.filter(e => filterNature === "All" || (e.nature || []).some(n => n.startsWith(filterNature))).map((e, i) => (
                        <tr key={e.id || i} style={{ borderBottom: `1px solid ${C.border}` }}>
                          ...
                          <td style={{ padding: "12px" }}>
                            {e.status ? <span style={styles.statusPill(e.status)}>{e.status}</span> : <span style={{ fontSize: 11, color: C.textLight }}>Pending</span>}
                          </td>
                          <td style={{ padding: "12px" }}>
