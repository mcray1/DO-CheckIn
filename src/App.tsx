    ...
    <div style={{ position: "relative" }}>
      <input
        ...
        style={{
          ...
          borderRadius: 10, padding: "14px 16px", fontSize: 17, color: "#f1f5f9",
      ...
      {(loading || results.length > 0) && (
        <div style={{
          ...
          zIndex: 50, overflow: "hidden", boxShadow: "0 8px 32px rgba(0,0,0,0.4)",
        }}>
...
// ── Hall Pass / Slip Preview ──────────────────────────────────────
function SlipPreview({ slip, onDone }) {
  return (
    <div style={{
      background: "#fff", borderRadius: 16, padding: "32px 36px", width: "100%",
      maxWidth: 480, boxShadow: "0 20px 60px rgba(0,0,0,0.5)", color: "#0f172a",
    }}>
      {/* Header */}
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {STATUS_OPTIONS.map(s => (
            <span key={s} style={{
              ...
              background: slip.status === s ? "#0f172a" : "#f1f5f9",
              color: slip.status === s ? "#fff" : "#94a3b8",
              border: "1px solid " + (slip.status === s ? "#0f172a" : "#e2e8f0"),
            }}>{s}</span>
          ))}
        </div>
...
export default function App() {
  const [view, setView] = useState("checkin"); // checkin | slip | log
  ...
  function validate() {
    ...
    if (!selectedStudent?.grade_section) e.section = "Grade & Section missing in database";
  ...
  const s = {
    ...
    natureBtn: (active) => ({
      padding: "10px 14px", border: "2px solid " + (active ? "#f59e0b" : "#334155"),
      background: active ? "rgba(245,158,11,0.12)" : "#0f172a",
    ...
    statusPill: (s) => {
      ...
      const [color, bg] = map[s] || ["#64748b","rgba(100,116,139,0.12)"];
      ...
      <div style={s.main}>
        ...
        {view === "checkin" && step === "search" && (
          <div style={s.card}>
            ...
            <StudentSearch onSelect={s => { setSelectedStudent(s); setStep("form"); }} />
          </div>
        )}
          ...
          <div style={s.card}>
            ...
            <div style={{ background: "#0f172a", border: "1px solid #334155", borderRadius: 10, padding: "12px 16px", marginBottom: 24, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                ...
                <div style={{ fontSize: 13, color: "#64748b" }}>{selectedStudent?.student_id} · {selectedStudent?.grade_section || <span style={{ color: "#ef4444" }}>No section in DB</span>}</div>
              </div>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  {["A.M.", "P.M."].map(m => (
                    <button key={m} onClick={() => setMeridiem(m)} style={{
                      flex: 1, padding: "10px", border: "2px solid " + (meridiem === m ? "#f59e0b" : "#334155"),
                      background: meridiem === m ? "rgba(245,158,11,0.12)" : "#0f172a",
                      color: meridiem === m ? "#f59e0b" : "#64748b",
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <textarea
                ...
                placeholder="Student explains the reason for this violation..."
                rows={4}
                style={{
                  width: "100%", background: "#0f172a",
                  border: "1.5px solid " + (reason.trim().length >= 10 ? "#f59e0b" : "#334155"),
                  borderRadius: 8, padding: "12px 14px", fontSize: 14, color: "#f1f5f9",
          ...
          <div style={{ background: "#1e293b", borderRadius: 16, padding: "28px 32px", width: "100%", maxWidth: 1000, boxShadow: "0 20px 60px rgba(0,0,0,0.5)" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
              ...
              <button onClick={() => { setLoadingLog(true); sbFetchSlips().then(setLog).catch(() => {}).finally(() => setLoadingLog(false)); }}
                style={{ background: "#0f172a", border: "1px solid #334155", color: "#94a3b8", borderRadius: 6, padding: "6px 14px", fontSize: 13, cursor: "pointer" }}>
                🔄 Refresh
              </button>
            </div>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
              {["All", ...NATURE_OPTIONS].map(f => (
                <button key={f} onClick={() => setFilterNature(f)} style={{
                  background: filterNature === f ? "#f59e0b" : "#0f172a",
                  color: filterNature === f ? "#0f172a" : "#64748b",
                  border: "1px solid " + (filterNature === f ? "#f59e0b" : "#334155"),
            ...
            {loadingLog ? (
              ...
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  ...
                  <tbody>
                    {log
                      .filter(e => filterNature === "All" || (e.nature || []).some(n => n.startsWith(filterNature)))
                      .map((e, i) => (
                          ...
                          <td style={{ padding: "10px", borderBottom: "1px solid #1e293b" }}>
                            <div style={{ display: "flex", gap: 4 }}>
                              {STATUS_OPTIONS.map(st => (
                                <button key={st} disabled={updatingId === e.id || e.status === st}
                                  onClick={() => handleStatusUpdate(e.id, st)}
                                  style={{
                                    padding: "3px 8px", fontSize: 10, fontWeight: 700, borderRadius: 4, cursor: "pointer", whiteSpace: "nowrap",
                                    background: e.status === st ? "#334155" : "#0f172a",
                                    color: e.status === st ? "#64748b" : "#94a3b8",
                                    border: "1px solid #334155",
                                    opacity: updatingId === e.id ? 0.5 : 1,
                                  }}>{st === "Admit Temporarily" ? "Admit Temp." : st}</button>
                              ))}
                            </div>
                          </td>
                        </tr>
